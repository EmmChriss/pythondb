...
def main():
    def create_folder():
        ...
        if(source_path==""):
            error_window=Tk()
            lbl_error = Label(error_window,text="No cancel pls", fg='red', font=('Times New Roman', 16), bg="lightblue")
    ...
    def create_column():
        ...
        if(oszlop_index=='foreign-key'):
            ...
            with open(referencia, 'r') as g:
                data=json.load(g)
            for i in data:
                if(i['column role']=='primary-key-unique' or i['column role']=='primary-key-not-unique'):
                    okes=1
                    break
                else:
                    okes=0
        ...
        if(oszlop_index == 'foreign-key'):         
            ...
            fk = oszlop_nev+' '+oszlop_index + ' ' + referencia
        else:
            data = {"column name":oszlop_nev,"column type":oszlop_tipus,"column role":oszlop_index}
            pk=oszlop_nev+' '+oszlop_index
        json.dump(data,f,indent=4)
        f.write("\n]")
        f.close()
        seged = source_path.split("/")  
        if(oszlop_index == 'primary-key-unique' or oszlop_index == 'primary-key-not-unique'):
            mongo.add_column(pk, seged[len(seged)-2], seged[len(seged)-1])
        elif(oszlop_index=='foreign-key'):
            mongo.add_column(fk, seged[len(seged)-2], seged[len(seged)-1])
